...
class PergoNewsletter extends Email_Subscribers{
	...
	public static function newsletter_form_button_field($data, $unique_id){		
        ...
        $button_html =  $before.PergoVcMapButtons::single_button_html($data, 'es_subscription_form_submit es_submit_button'.$extra_class, 'button', $btn_atts).$after;
    	}else{
    		extract($data);
	...
	public static function es_get_form_horizontal( $instance ) {
		...
		$es_desc  = ! empty( $instance['es_desc'] ) ? $instance['es_desc'] : '';		
		$es_name  = ! empty( $instance['enable_name'] ) ? $instance['enable_name']: false;
		$es_group = ! empty( $instance['es_group'] ) ? $instance['es_group'] : '';
		$es_pre   = ! empty( $instance['es_pre'] ) ? $instance['es_pre'] : '';
		$name_placeholder  = ! empty( $instance['name'] ) ? $instance['name'] : '';
		$email_placeholder  = ! empty( $instance['email'] ) ? $instance['email'] : '';
		$email_wrapper_class = ($es_name == "yes")? 'col-lg-5' : 'col-lg-9';

		$instance['form_class'] = 'quick-form-horizontal form-holder register-form mb-30';	
	...
	public static function es_get_form_simple( $instance ) {
		...
		$button_style = ! empty( $instance['button_style'] ) ?$instance['button_style'] : 'btn-preset';
		$button_style = str_replace("btn-","",$button_style).'-color';
	...
	public function create_newsletter_group(){	
		...
		if( $count == 0 ) $id = $wpdb->insert( $table, $data );		
